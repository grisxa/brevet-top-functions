...
from strava.api import auth_token, get_activities, get_track_points
from strava.exceptions import ActivityNotFound, ActivityError
...
def track_alignment(
    ...
    cost, reduced = np_align_track_to_route(np.array(brevet.get("short_track", [])), shortened)
    ...
    cost_reviewed = np_geo_distance_track(
        np.array(brevet["short_track"]), reduced, factor=0
    )
    track_time = timer()
    logging.info(
    ...
    # re-calculate the cost ignoring distance from the start
    cost_reviewed = np_geo_distance_track(np.array(checkpoints), reduced, 0)
    cp_time = timer()
    logging.info(
